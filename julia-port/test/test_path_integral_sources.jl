...
@testset "Fiber source API" begin
    ...
    spec = FiberSpec(0.0, 20.0; cross_section = DEMO_FIBER_CROSS_SECTION, λ_m = DEMO_λ_M)
    twist!(spec, 0.0, 5.0; rate = 0.15, T_K = DEMO_T_K)
    bend!(spec, 0.0, 2.0; angle = π / 2, axis = 0.0, T_K = DEMO_T_K)
    bend!(spec, 2.0, 3.0; angle = 0.0, axis = 0.0, T_K = DEMO_T_K)
    twist!(spec, 5.0, 7.0; rate = -0.05, T_K = DEMO_T_K)
    bend!(spec, 3.0, 4.0; angle = π / 4, axis = π / 6, T_K = DEMO_T_K)
    bend!(spec, 4.0, 7.0; angle = 0.0, axis = π / 6, T_K = DEMO_T_K)
