...
function core_refractive_index(style::SpectralStyle, fiber::FiberCrossSection, λ, T_K)
    return refractive_index(style, fiber.core_material, λ, T_K)
end

core_refractive_index(fiber::FiberCrossSection, λ, T_K) =
    core_refractive_index(ValueOnly(), fiber, λ, T_K)

function cladding_refractive_index(style::SpectralStyle, fiber::FiberCrossSection, λ, T_K)
    return refractive_index(style, fiber.cladding_material, λ, T_K)
end

cladding_refractive_index(fiber::FiberCrossSection, λ, T_K) =
...
function mode_terms(::ValueOnly, fiber::FiberCrossSection, λ, T_K)
...
function mode_terms(::WithDerivative, fiber::FiberCrossSection, λ, T_K)
...
function guided_refractive_indices(style::ValueOnly, fiber::FiberCrossSection, λ, T_K)
    terms = mode_terms(style, fiber, λ, T_K)
    return terms.n_core, terms.n_clad
end

function guided_refractive_indices(style::WithDerivative, fiber::FiberCrossSection, λ, T_K)
    terms = mode_terms(style, fiber, λ, T_K)
    return (
...
guided_refractive_indices(fiber::FiberCrossSection, λ, T_K) =
    guided_refractive_indices(ValueOnly(), fiber, λ, T_K)

function relative_index_difference(style::ValueOnly, fiber::FiberCrossSection, λ, T_K)
    n_core, n_clad = guided_refractive_indices(style, fiber, λ, T_K)
    return (n_core - n_clad) / n_clad
end

relative_index_difference(fiber::FiberCrossSection, λ, T_K) =
    relative_index_difference(ValueOnly(), fiber, λ, T_K)

numerical_aperture(style::ValueOnly, fiber::FiberCrossSection, λ, T_K) =
    mode_terms(style, fiber, λ, T_K).na

function numerical_aperture(style::WithDerivative, fiber::FiberCrossSection, λ, T_K)
    terms = mode_terms(style, fiber, λ, T_K)
    return SpectralResponse(terms.na, terms.dna_dω)
end

numerical_aperture(fiber::FiberCrossSection, λ, T_K) =
    numerical_aperture(ValueOnly(), fiber, λ, T_K)

normalized_frequency(style::ValueOnly, fiber::FiberCrossSection, λ, T_K) =
    mode_terms(style, fiber, λ, T_K).V

function normalized_frequency(style::WithDerivative, fiber::FiberCrossSection, λ, T_K)
    terms = mode_terms(style, fiber, λ, T_K)
    return SpectralResponse(terms.V, terms.dV_dω)
end

normalized_frequency(fiber::FiberCrossSection, λ, T_K) =
    normalized_frequency(ValueOnly(), fiber, λ, T_K)

propagation_constant(style::ValueOnly, fiber::FiberCrossSection, λ, T_K) =
    mode_terms(style, fiber, λ, T_K).β

function propagation_constant(style::WithDerivative, fiber::FiberCrossSection, λ, T_K)
    terms = mode_terms(style, fiber, λ, T_K)
    return SpectralResponse(terms.β, terms.dβ_dω)
end

propagation_constant(fiber::FiberCrossSection, λ, T_K) =
    propagation_constant(ValueOnly(), fiber, λ, T_K)

function effective_mode_index(style::ValueOnly, fiber::FiberCrossSection, λ, T_K)
    terms = mode_terms(style, fiber, λ, T_K)
    return terms.β / terms.k0
end

function effective_mode_index(style::WithDerivative, fiber::FiberCrossSection, λ, T_K)
    terms = mode_terms(style, fiber, λ, T_K)
...
effective_mode_index(fiber::FiberCrossSection, λ, T_K) =
...
function effective_mode_area(fiber::FiberCrossSection, λ, T_K)
...
core_nonlinear_refractive_index(fiber::FiberCrossSection, λ, T_K) =
    nonlinear_refractive_index(fiber.core_material, λ, T_K)

function nonlinear_coefficient(fiber::FiberCrossSection, λ, T_K)
    n2 = core_nonlinear_refractive_index(fiber, λ, T_K)
    Aeff = effective_mode_area(fiber, λ, T_K)
    return (2π / λ) * n2 / Aeff
end

is_single_mode(fiber::FiberCrossSection, λ, T_K) =
...
function core_noncircularity_dω(style::SpectralStyle, fiber::FiberCrossSection, λ, T_K; axis_ratio)
    terms = mode_terms(style, fiber, λ, T_K)
    ε = validate_axis_ratio(axis_ratio)
...
core_noncircularity_birefringence(::ValueOnly, fiber::FiberCrossSection, λ, T_K; axis_ratio) =
    core_noncircularity_dω(ValueOnly(), fiber, λ, T_K; axis_ratio = axis_ratio).Δβ

core_noncircularity_birefringence(::WithDerivative, fiber::FiberCrossSection, λ, T_K; axis_ratio) =
    core_noncircularity_dω(WithDerivative(), fiber, λ, T_K; axis_ratio = axis_ratio)

core_noncircularity_birefringence(fiber::FiberCrossSection, λ, T_K; axis_ratio) =
    core_noncircularity_birefringence(ValueOnly(), fiber, λ, T_K; axis_ratio = axis_ratio)

asymmetric_thermal_stress_birefringence(::ValueOnly, fiber::FiberCrossSection, λ, T_K; axis_ratio) =
    asymmetric_thermal_stress_dω(ValueOnly(), fiber, λ, T_K; axis_ratio = axis_ratio).Δβ

asymmetric_thermal_stress_birefringence(::WithDerivative, fiber::FiberCrossSection, λ, T_K; axis_ratio) =
    asymmetric_thermal_stress_dω(WithDerivative(), fiber, λ, T_K; axis_ratio = axis_ratio)

asymmetric_thermal_stress_birefringence(fiber::FiberCrossSection, λ, T_K; axis_ratio) =
    asymmetric_thermal_stress_birefringence(ValueOnly(), fiber, λ, T_K; axis_ratio = axis_ratio)

bending_birefringence(::ValueOnly, fiber::FiberCrossSection, λ, T_K; bend_radius_m) =
    bending_dω(ValueOnly(), fiber, λ, T_K; bend_radius_m = bend_radius_m).Δβ

bending_birefringence(::WithDerivative, fiber::FiberCrossSection, λ, T_K; bend_radius_m) =
    bending_dω(WithDerivative(), fiber, λ, T_K; bend_radius_m = bend_radius_m)

bending_birefringence(fiber::FiberCrossSection, λ, T_K; bend_radius_m) =
    bending_birefringence(ValueOnly(), fiber, λ, T_K; bend_radius_m = bend_radius_m)

axial_tension_birefringence(::ValueOnly, fiber::FiberCrossSection, λ, T_K; bend_radius_m, axial_tension_N) =
    axial_tension_dω(ValueOnly(), fiber, λ, T_K; bend_radius_m = bend_radius_m, axial_tension_N = axial_tension_N).Δβ

axial_tension_birefringence(::WithDerivative, fiber::FiberCrossSection, λ, T_K; bend_radius_m, axial_tension_N) =
    axial_tension_dω(WithDerivative(), fiber, λ, T_K; bend_radius_m = bend_radius_m, axial_tension_N = axial_tension_N)

axial_tension_birefringence(fiber::FiberCrossSection, λ, T_K; bend_radius_m, axial_tension_N) =
    axial_tension_birefringence(ValueOnly(), fiber, λ, T_K; bend_radius_m = bend_radius_m, axial_tension_N = axial_tension_N)

twisting_birefringence(::ValueOnly, fiber::FiberCrossSection, λ, T_K; twist_rate_rad_per_m) =
    twisting_dω(ValueOnly(), fiber, λ, T_K; twist_rate_rad_per_m = twist_rate_rad_per_m).Δβ

twisting_birefringence(::WithDerivative, fiber::FiberCrossSection, λ, T_K; twist_rate_rad_per_m) =
    twisting_dω(WithDerivative(), fiber, λ, T_K; twist_rate_rad_per_m = twist_rate_rad_per_m)

twisting_birefringence(fiber::FiberCrossSection, λ, T_K; twist_rate_rad_per_m) =
